...
export default function Home() {
  ...
  const displayedFaculty =
    facultyFilter === 'all'
      ? faculty
      : faculty.filter((f) => f.category === facultyFilter)

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentFaculty((prev) => (prev + 1) % displayedFaculty.length)
    }, 4500)
  ...
  const prevFaculty = () => {
    setCurrentFaculty((prev) => (prev - 1 + displayedFaculty.length) % displayedFaculty.length)
  }
      ...
      <section className="faculty-section section" id="faculty">
        <div className="faculty-container">
          <div className="section-heading">
            <div className="sec-pill" style={{ marginBottom: '8px' }}>EXPERT MENTORSHIP PANEL</div>
            <h2>Our Faculty & Defence Mentors</h2>
            <p className="faculty-subtitle">
              Guided by experienced subject specialists and veteran SSB Assessors committed to concept clarity and officer selection.
            </p>
          </div>
          ...
          <div className="faculty-filter-tabs">
            <button
              type="button"
              className={`faculty-filter-btn ${facultyFilter === 'all' ? 'active' : ''}`}
              onClick={() => {
                setFacultyFilter('all')
                setCurrentFaculty(0)
              }}
            >
              All Mentors ({faculty.length})
            </button>
            <button
              type="button"
              className={`faculty-filter-btn ${facultyFilter === 'Defence Services' ? 'active' : ''}`}
              onClick={() => {
                setFacultyFilter('Defence Services')
                setCurrentFaculty(0)
              }}
            >
              Defence Services & SSB Assessors (3)
            </button>
            <button
              type="button"
              className={`faculty-filter-btn ${facultyFilter === 'Academic Faculty' ? 'active' : ''}`}
              onClick={() => {
                setFacultyFilter('Academic Faculty')
                setCurrentFaculty(0)
              }}
            >
              Academic Subject Faculty (4)
            </button>
          </div>
          ...
          <div className="faculty-cta-wrapper" style={{ textAlign: 'center', marginTop: '40px' }}>
            <Link to="/services/defence-preparation" className="nav-btn" style={{ padding: '15px 40px', fontSize: '1.1rem' }}>
              Explore Defence Services
            </Link>
          </div>
        </div>
      </section>
